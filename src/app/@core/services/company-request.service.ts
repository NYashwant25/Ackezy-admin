...
export class CompanyRequestService {
  ...
  constructor(private http: HttpClient) {
    ...
    filterDocs(fieldParameters): any{
        return this.http.post<any>(`${environment.apiUrl}/dashboard/document/search`, fieldParameters)
                .pipe(map(response => {
                    if (response.code === 200) {
                        console.log(response.data.data);
                    }
    ...
    getType() {
        return this.http.get<any>(`${environment.apiUrl}/company/getType`)
                .pipe(map(response => {
    ...
    getDocument() {
        return this.http.get<any>(`${environment.apiUrl}/dashboard/document/getall`)
                .pipe(map(response => {
