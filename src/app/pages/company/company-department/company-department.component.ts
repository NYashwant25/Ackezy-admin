...
export class CompanyDepartmentComponent implements OnInit {
	...
	departData = { 'name': '', 'description': '', head: '', companyName: '', 'companyId': '', IsActive: true, id: '' };
	...
	ngOnInit() {
		...
		this.dtOptions = {
			...
			ajax: (dataTablesParameters: object, callback) => {
				const responseData = this.companyService.getDepartmentData({ companyId: this.comID }).pipe(first())
					.subscribe(response => {
	...
	getSinCompany(data) {
		this.commonService.getData(data, "Company").subscribe(response => {
			if (response.code === 200) {
	...
	updateRequest(departForm) {
		...
		console.log(this.departData)
		this.departData.companyName = this.companyList.name;
		this.departData.companyId = this.companyList._id;
